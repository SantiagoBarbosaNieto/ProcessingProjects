class Dot {
  ...
  void update() {
    if (!dead && !reachedGoal) {
      ...
      if (pos.x< 2|| pos.y<2 || pos.x>width-2 || pos.y>height -2) {//if near the edges of the window then kill it 
        dead = true;
      } else if (dist(pos.x, pos.y, goal1.x, goal1.y) < 5) {//if reached goal

        reachedGoal = true;
      } else if (pos.x< 600 && pos.y < 210 && pos.x > 0 && pos.y > 200) {//if hit obstacle
        dead = true;
      } else if(pos.x<800 && pos.y < 410 && pos.x > 300 && pos.y > 400){
        dead = true;
      }else if( pos.x < 500 && pos.y < 610 && pos.x > 0 && pos.y > 600)
      {
        dead = true;
      }
    }
  ...
  void calculateFitness() {
    if (reachedGoal) {//if the dot reached the goal then the fitness is based on the amount of steps it took to get there
      ...
      float distanceToGoal = dist(pos.x, pos.y, goal1.x, goal1.y);
